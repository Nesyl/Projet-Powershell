####################################
#CREATION D'OU DE MANIERE RECURSIVE#
####################################
...
ForEach( $NomOU in $FilialeOU ){
    If (Get-ADOrganizationalUnit -Filter 'Name -eq $NomOU') { # On vérifie si l'OU que l'on souhaite créer existe déjà
        Write-warning "L'OU $NomOU existe déjà `n" # On alerte de l'existence de l'OU
    ...
    Else {
        ...
        Write-Host "Création de l'OU $NomOU `n" # On indique la création de l'OU
        Start-Sleep -Seconds 1 # Temps de pause d'une seconde afin de faciliter la lecture du script durant l'exécution
    }
}
...
ForEach( $NomOU in $ServicesOU ){
    If (Get-ADOrganizationalUnit -Filter 'Name -eq $NomOU') { # On vérifie si l'OU que l'on souhaite créer existe déjà
        Write-warning "L'OU $NomOU existe déjà `n" # On indique la création de l'OU
        Start-Sleep -Seconds 1 # Temps de pause d'une seconde afin de faciliter la lecture du script durant l'exécution
    }
    Else {
        New-ADOrganizationalUnit -Name $NomOU  ` # Dans le cas où l'OU n'existe pas, on la créée en indiquant son emplacement et en la protégeant d'une suppression accidentelle
...
ForEach( $NomOU in $RechercheOU ){
    If (Get-ADOrganizationalUnit -Filter 'Name -eq $NomOU') { #On vérifie si l'OU que l'on souhaite créer existe déjà
        Write-warning "L'OU $NomOU existe déjà `n" #Si l'OU existe déjà on alerte l'utilisateur 
        Start-Sleep -Seconds 1 #Temps de pause d'une seconde afin de faciliter la lecture du script durant l'exécution
    }
    Else {
        New-ADOrganizationalUnit -Name $NomOU  ` #Dans le cas où l'OU n'existe pas, on la créée en indiquant son emplacement et en la protégeant d'une suppression accidentelle
        ...
        -ProtectedFromAccidentalDeletion $true
        Write-Host "Création de l'OU $NomOU `n" #On prévient l'utilisateur de la création de l'OU
        Start-Sleep -Seconds 1 #Temps de pause d'une seconde afin de faciliter la lecture du script durant l'exécution
    }
}
